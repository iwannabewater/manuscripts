...
[fetch] tier=local status=fail reason="fetch failed: HTTP Error 403: Forbidden"
[fetch] tier=defuddle status=ok
---
title: "Unlocking the Codex harness: how we built the App Server"
author: "Celia Chen, Member of the Technical Staff"
site: "OpenAI"
...
Codex Web uses the Codex harness, but runs it in a container environment. A worker provisions a container with the checked-out workspace, launches the App Server binary inside it, and maintains a long-lived JSON-RPC over stdio [^2] channel. The web app (running in the user’s browser tab) talks to the Codex backend over HTTP and SSE, which streams task events produced by the worker. This keeps the browser-side UI lightweight while still giving us a consistent runtime across desktop and web.

...
正文资料口径：截至 `2026-06-02` 的公开资料；平台状态于 `2026-06-09` 复核。Apple 已发布 macOS 27 与 Xcode 27 beta，但稳定工作站基线仍按 `macOS Tahoe 26.5` 编写。软件界面、许可和系统菜单会变化，安装前应再次阅读对应官方页面。
...
WWDC26 后，Apple 已提供 macOS 27 与 Xcode 27 beta。需要验证新 SDK 的开发者应使用独立 APFS volume、备用设备或可恢复的测试环境，不应直接把唯一主力工作站升级为 beta。Xcode 27 beta 要求 macOS Tahoe 26.4 或更高版本；不需要新 SDK 的团队继续使用稳定系统和稳定 Xcode。

# Tips & Tricks | mise-en-place

> Source: https://mise.jdx.dev/tips-and-tricks.html
...
sh$ curl https://mise.run | MISE_INSTALL_PATH=~/.local/bin/mise-x64 MISE_INSTALL_ARCH=x64 sh
...
shcurl https://mise.run > setup-mise.sh
...
shzinit as="command" lucid from="gh-r" for \
id-as="usage" \
atpull="%atclone" \
jdx/usage
...
shmise set NODE_ENV=production

Using Tera to read unsupported version files ​

Some project-local version files are already supported as idiomatic version files. For other version files, you can use Tera templates in mise.toml to read the file and assign the version to the appropriate tool.
...
See Templates for more details on Tera functions and filters.
...
sh# Disable specific verification methods if needed
...
To limit supply chain risk, you can restrict mise to only install versions released before a certain date or duration. This is similar to Renovate's minimum release age concept — newly published versions are ignored until they've been available for a configurable amount of time.
...
For npm: and pipx: tools, the same cutoff is also forwarded to transitive dependency resolution during install. Refer to the npm backend docs and pipx backend docs for package-manager support details.
...
See minimum_release_age for more details.

mise up --bump ​

Use mise up --bump to upgrade all software to the latest version and update mise.toml files. This keeps the same semver range as before, so if you had node = "24" and node 26 is the latest, mise up --bump node will change mise.toml to node = "26".
...
When lockfiles are enabled, mise will update mise.lock with full versions and tarball checksums (if supported by the backend). These can be updated with mise up. You need to manually create the lockfile, then mise will add the tools to it:
...
No need for GITHUB_TOKEN: Since the URLs are already known, you don’t need to set up a GITHUB_TOKEN for simple installs. See GitHub Tokens for more on token configuration.

...
shPATH="$HOME/.local/share/mise/installs/python/3.15.0/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"

In this example, the python bin directory was added at the beginning of the PATH, making it available in the current shell session. When a fuzzy version like python = "3.15" or node = "26" is active, this path may use the requested-version symlink, such as ~/.local/share/mise/installs/python/3.15/bin, instead of the fully resolved patch version.
...
shls -l ~/.local/share/mise/shims/node
...
shexport PATH="$HOME/.local/share/mise/shims:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
...
sh# note that bash will read from ~/.profile or ~/.bash_profile if the latter exists
...
shecho 'eval "$(mise activate zsh --shims)"' >> ~/.zprofile # this sets up non-interactive sessions
echo 'eval "$(mise activate zsh)"' >> ~/.zshrc    # this sets up interactive sessions

shecho 'mise activate fish --shims | source' >> ~/.config/fish/config.fish
...
sh$ mise set NODE_ENV=production
$ echo $NODE_ENV
production
...
In other words, which is better in terms of performance just depends on how you're calling mise. Really though most users will not notice a few ms lag on their terminal caused by mise activate. See Troubleshooting: Slow shell prompts for how to diagnose performance issues.
...
sheval "$(mise activate zsh)"
eval "$(mise hook-env -s zsh)"
node some_script.js

sheval "$(mise activate zsh --shims)" # should be first
